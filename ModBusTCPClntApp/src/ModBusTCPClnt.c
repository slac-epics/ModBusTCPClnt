 ...
 * we need those prototypes separated from the cloned header
 * code in SocketWithTimeout.h
 */
extern unsigned int ipAddrToA(	const struct sockaddr_in * paddr,
...
ModBusTCP_Link MBT_Init( const char * deviceName,  const char * deviceIP, unsigned short int port, unsigned char UNIT)
{
	...
	{
		printf("No deviceName specified. Check your initialization parameters!\n");
		return	NULL;
	}

	if( deviceIP == NULL || strlen(deviceIP) == 0 )
	{
		printf("No deviceIP addr specified. Check your initialization parameters!\n");
		return  NULL;
	}
	if( inet_addr(deviceIP) == INADDR_NONE )
	{
		printf("deviceIP addr %s is invalid. Check your initialization parameters!\n", deviceIP );
		return  NULL;
	}

	if( UNIT != MBT_UNIT_ENDPOINT && UNIT != MBT_UNIT_GTW_BC && (UNIT < MBT_UNIT_GTW_MIN || UNIT > MBT_UNIT_GTW_MAX) )
	{
		printf("UNIT %u is invalid. Check your initialization parameters!\n", UNIT );
		return  NULL;
	}
...
int MBT_Connect(ModBusTCP_Link mbt_link, unsigned int toutsec)
{
	...
	if(mbt_link == NULL)
	{
		printf("Invalid ModBusTCP_Link ptr!\n");
		return -1;
	}
		...
		if (status == SOCKET_ERROR)
		{
			...
			{	/* Normally this means connection was refused, nobody listen on this port */
				...
				if ( 1 )	/* if(MBT_DRV_DEBUG) */
				{
					char tmp_info[80];
					char acSockAddr[80];
					ipAddrToA( &mbt_link->addr, acSockAddr, 80 );
					sprintf(tmp_info, "ModBusTCP connection to device %s at %s was refused!\n",
							mbt_link->name, acSockAddr );
					perror(tmp_info);
				}
			}
			else
			{
				...
				{
					char tmp_info[80];
					sprintf(tmp_info, "ModBusTCP connection to device %s timed out!\n", mbt_link->name);
					perror(tmp_info);
				}
			}
			socket_close(mbt_link->sFd);
...
int MBT_Disconnect(ModBusTCP_Link mbt_link, unsigned int errCode)
{
	...
	if(mbt_link->linkStat==LINK_OK)
	{
		...
		{
			char	errMsg[40];
			MBT_ErrCodeToString( mbt_link->lastErr, errMsg, 40 );
			printf( "Disconnecting %s.  errCode 0x%08x: %s\n", mbt_link->name, mbt_link->lastErr, errMsg );
		}
	}
	else
	{
...
static int MBT_Read_Response(
	ModBusTCP_Link			mbt_link,
	unsigned char			expect_fcode,
	unsigned char		**	ppPDUbuf,
	unsigned int		*	pPDUsize,
	struct timeval		*	ptimeout	)
{
	...
	status = readWithTimeout(mbt_link->sFd, (char *) pPDUbuf, PDUsize, ptimeout);
	if(status == SOCKET_ERROR)
	{
	...
	{/* We got an exception PDU */
		...
		{
			char	errMsg[40];
			MBT_ErrCodeToString(mbt_excpt_pdu->errcode, errMsg, 40);
			printf("MBT_Read_Response: Remote err code %d: %s\n",mbt_excpt_pdu->errcode, errMsg);
		}
...
/* int MBT_Read_Response(ModBusTCP_Link mbt_link, unsigned char expect_fcode, unsigned char **ppPDUbuf, unsigned int * pPDUsize, struct timeval *ptimeout) */
	...
	if (	( pmbt_f1_res->RByteCount != (int)((RBitCount+7)/8) )
		||	( mbt_f1_res_size != (pmbt_f1_res->RByteCount + 2) )	)
...
/* int MBT_Write_Request(ModBusTCP_Link mbt_link, unsigned char *pPDUbuf, unsigned int PDUsize, struct timeval *ptimeout) */
	...
	if (	( pmbt_f2_res->RByteCount != (int)((RBitCount+7)/8)	)
		||	( mbt_f2_res_size != (pmbt_f2_res->RByteCount + 2)	)	)
	{/* +2 is fCode and RByteCount himself */
		MBT_Disconnect(mbt_link, MBT_ERR_DATA_SIZE);
		free(pmbt_f2_res);
		SEM_GIVE(mbt_link->semMbt);
...
int MBT_Function3(
	ModBusTCP_Link mbt_link,
	unsigned short int wRIORoffset,
...
int MBT_Function16(ModBusTCP_Link mbt_link, unsigned short int wWORoffset, unsigned short int WWordCount, unsigned short int *pWWordData, unsigned int toutsec)
{
	...
	for(loop=0;loop<WWordCount;loop++)
		pmbt_f16_req->WWordData[loop] = htons(pWWordData[loop]);
...
int MBT_Test(char * IPAddr)
{
	...
	printf("\nReset watchdog to original timeout of %dms\n", WDdataOrig);
	MBT_Function6(mbt_link, 0x1120, WDdataOrig, 2);
